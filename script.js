...
function showResult() {
    ...
    const name = document.getElementById('name').value; // Assuming the input field for name has an ID of 'name'
    console.log("Total Score:", totalScore);
    console.log("Option Click Counts:", optionClickCounts);
    
    // Logic to check score and option clicks
    if (totalScore >= 145) {
        const optionLessThan6 = Object.values(optionClickCounts).some(count => count < 6);
        console.log("Option Less Than 6:", optionLessThan6);
        if (optionLessThan6) {
            // Display message with Restart Button
            console.log("Displaying restart message...");
            quizContainer.innerHTML = `Dear ${name}, Score: ${totalScore}  Count: ${count} তোমার মাঝে অফুরন্ত সম্ভাবনা রয়েছে।কিন্তু তোমার দেওয়া উত্তরগুলোতে
            <button class="btn" onclick="location.reload()">Restart</button>`;
        } else {
            // Display congratulatory message
            console.log("Displaying congratulatory message...");
            quizContainer.innerHTML = `অনেক অনেক অভিনন্দন প্রিয় ${name}, তুমি ঠিক পথেই এগিয়ে চলেছ। তোমার পেশাগত লক্ষ্য`;
            // Generate PDF and store it in the database
            generateAndStorePDF(name);
        }
    } else {
        const optionMoreThan6 = Object.values(optionClickCounts).some(count => count >= 6);
        const optionLessThan6 = Object.values(optionClickCounts).some(count => count < 6);
        console.log("Option More Than 6:", optionMoreThan6);
        console.log("Option Less Than 6:", optionLessThan6);
        if (optionMoreThan6) {
            // Display message about powerful
            console.log("Displaying powerful message...");
            quizContainer.innerHTML = `Dear ${name}, তোমার মাঝে অফুরন্ত সম্ভাবনা রয়েছে। অনেক ক্ষেত্রে তুমি অত্যন্ত শক্তিশালী কিন্তু`;
            // Generate PDF and store it in the database
            generateAndStorePDF(name);
        } else if (optionLessThan6) {
            // Display message about potential success
            console.log("Displaying potential success message...");
            quizContainer.innerHTML = `Dear ${name}, তোমার মাঝে অফুরন্ত সম্ভাবনা রয়েছে এবং তুমি চেষ্টা করলে নিশ্চিতভাবেই একজন অত্যন্ত সফল……… 6 A<B<C<D হতে পারবে।`;
            // Generate PDF and store it in the database
            generateAndStorePDF(name);
        }
    }
...
function generateAndStorePDF(name) {
    const doc = new jsPDF();
    // Add content to the PDF
    doc.text(`Dear ${name},`, 10, 10);
    doc.text('Congratulations! You have completed the quiz.', 10, 20);
    // Save the PDF
    const pdfData = doc.output('datauristring');
    // Send the PDF data to your backend for storage
    // Example: sendPDFToBackend(pdfData);
}

function sendPDFToBackend(pdfData) {
    // Send the PDF data to your backend using AJAX or fetch
    // Example:
    fetch('/store-pdf', {
        method: 'POST',
